...
lab.experiment('models.users.replace(username, payload, currentUser)', () => {
  ...
  lab.experiment('username and currentUser do not match', () => {
    ...
    lab.test('Should reject with a "permission" error', () => {
      ...
      return server.plugins.models.users.replace(mismatchedUsername, validPayload, mismatchedCurrentUser)
        .then(() => {
  ...
  lab.experiment('Malformed username (and currentUser)', () => {
    ...
    lab.test('Should reject with a "malformed" error', () => {
      ...
      return server.plugins.models.users.replace(malformedUsername, validPayload, malformedCurrentUser)
        .then(() => {
          throw new Error('Expected promise to reject');
        })
        .catch((err) => {
  ...
  lab.experiment('Malformed payload', () => {
    ...
    lab.test('Should reject with a "malformed" error', () => {
      ...
      return server.plugins.models.users.replace(validUsername, malformedPayload, validCurrentUser)
        .then(() => {
          throw new Error('Expected promise to reject');
        })
        .catch((err) => {
  ...
  lab.experiment('Nonexistent username (and currentUser)', () => {
    ...
    lab.test('Should reject with a "nonexistent" error', () => {
      ...
      return server.plugins.models.users.replace(nonexistentUsername, validPayload, nonexistentCurrentUser)
        .then(() => {
          throw new Error('Expected promise to reject');
        })
        .catch((err) => {
  ...
  lab.experiment('Valid Input', () => {
    ...
    lab.test('Should resolve with boolean true', () => {
      ...
      return server.plugins.models.users.replace(validUsername, validPayload, validCurrentUser)
        .then((data) => {
    ...
    lab.test('Should modify database as expected', () => {
      ...
      return helpers.testDatabaseChanges(config, func)
        .then((changes) => {
          ...
          Code.expect(changes[1].path).to.equal([ 'users', '1' , 'email_address']);
          Code.expect(changes[1].lhs).to.equal('homer@example.com');
          Code.expect(changes[1].rhs).to.equal('TheChosenOne@example.com');

          Code.expect(changes[2].kind).to.equal('E');
          Code.expect(changes[2].path).to.equal([ 'users', '1' , 'password_hash']);
        });
    });
  });
});
