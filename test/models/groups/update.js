...
lab.experiment('models.groups.update(groupID, payload, currentUser)', () => {
  ...
  lab.experiment('Malformed groupID', () => {
    ...
    lab.test('Should reject with a "malformed" error', () => {
      ...
      const validPayload     = { name: 'New group name' };
      const validCurrentUser = 'homer';

      return server.plugins.models.groups.update(malformedGroupID, validPayload, validCurrentUser)
        .then(() => {
  ...
  lab.experiment('Malformed payload', () => {
    ...
    lab.test('Should reject with a "malformed" error', () => {
      ...
      return server.plugins.models.groups.update(validGroupID, malformedPayload, validCurrentUser)
        .then(() => {
          throw new Error('Expected promise to reject');
        })
        .catch((err) => {
  ...
  lab.experiment('Malformed currentUser', () => {
    ...
    lab.test('Should reject with a "malformed" error', () => {
      ...
      return server.plugins.models.groups.update(validGroupID, validPayload, malformedCurrentUser)
        .then(() => {
          throw new Error('Expected promise to reject');
        })
        .catch((err) => {
  ...
  lab.experiment('Nonexistent groupID', () => {
    ...
    lab.test('Should reject with a "nonexistent" error', () => {
      ...
      return server.plugins.models.groups.update(nonexistentGroupID, validPayload, validCurrentUser)
        .then(() => {
          throw new Error('Expected promise to reject');
        })
        .catch((err) => {
  ...
  lab.experiment('Nonexistent currentUser', () => {
    ...
    lab.test('Should reject with a "nonexistent" error', () => {
      ...
      return server.plugins.models.groups.update(validGroupID, validPayload, nonexistentCurrentUser)
        .then(() => {
          throw new Error('Expected promise to reject');
        })
        .catch((err) => {
  ...
  lab.experiment('Group not owned by currentUser', () => {
    ...
    lab.test('Should reject with a "permission" error', () => {
      ...
      return server.plugins.models.groups.update(unownedGroupID, validPayload, validCurrentUser)
        .then(() => {
          throw new Error('Expected promise to reject');
        })
        .catch((err) => {
  ...
  lab.experiment('Valid Input (change name)', () => {
    ...
    lab.test('Should resolve with boolean true', () => {
      ...
      return server.plugins.models.groups.update(validGroupID, validPayload, validCurrentUser)
        .then((data) => {
    ...
    lab.test('Should modify database as expected', () => {
      ...
      const func = server.plugins.models.groups.update.bind(this, validGroupID, validPayload, validCurrentUser);
      ...
      return helpers.testDatabaseChanges(config, func)
        .then((changes) => {
          ...
          Code.expect(changes[0].path).to.equal([ 'groups', '1', 'name' ]);
  ...
  lab.experiment('Valid Input (change members)', () => {
    ...
    const validPayload     = { members: [ 'lenny', 'moe' ] };
    const validCurrentUser = 'homer';
  ...
  lab.experiment('Valid Input (duplicate members)', () => {
    ...
    const validGroupID     = 2;
    const validPayload     = { members: [ 'lenny', 'moe', 'lenny' ] };
    const validCurrentUser = 'homer';
